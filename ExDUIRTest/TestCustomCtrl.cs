...
namespace ExDuiRTest
{
    ...
    public class CefChromeBrowser : ExControl
    {
        ...
        private static void OnIMEComposition(IBrowserHost host, IntPtr hWnd, int lParam)
        {
            ...
            {
                host.ImeCommitText(text, new CefSharp.Structs.Range(int.MaxValue, int.MaxValue), 0);
                host.ImeSetComposition(text, new CompositionUnderline[0], new CefSharp.Structs.Range(int.MaxValue, int.MaxValue), new CefSharp.Structs.Range(0, 0));
            ...
            else
            {
                ...
                {
                    host.ImeSetComposition(text, underlines.ToArray(), new CefSharp.Structs.Range(int.MaxValue, int.MaxValue), new CefSharp.Structs.Range(compositionStart + underlines.Count, compositionStart + underlines.Count));
    ...
    public static class ImeHandler
    {
        ...
        private static void GetCompositionInfo(IntPtr hwnd, uint lParam, string text, List<CompositionUnderline> underlines, ref int compositionStart)
        {
            ...
            {
                var range = new CefSharp.Structs.Range();

                bool thick = false;

                if (targetStart > 0)
                {
                    range = new CefSharp.Structs.Range(0, targetStart);
                }

                if (targetEnd > targetStart)
                {
                    range = new CefSharp.Structs.Range(targetStart, targetEnd);
                    thick = true;
                }

                if (targetEnd < text.Length)
                {
                    range = new CefSharp.Structs.Range(targetEnd, text.Length);
                }
        ...
        private static void GetCompositionUnderlines(IntPtr hIMC, int targetStart, int targetEnd, List<CompositionUnderline> underlines)
        {
            ...
            for (int i = 0; i < clauseLength_1; i++)
            {
                ...
                var range = new CefSharp.Structs.Range(from, to);
