...
namespace DemoProject
{
    static class LoginWindow
    {
        ...
        {
            //读入主题包
            var theme = File.ReadAllBytes("Resources/Default.bin");
            ...
            if (skin.Validate)
            {
                ...
                var loginnormal = File.ReadAllBytes("Resources/loginnormal.png");
                var loginhover = File.ReadAllBytes("Resources/loginhover.png");
                ExImageInfo info1 = new ExImageInfo
                {
                    imgNormal = new ExImage(loginnormal, loginnormal.Length).handle,
                    imgHover = new ExImage(loginhover, loginhover.Length).handle,
                    imgDownOrChecked = new ExImage(loginnormal, loginnormal.Length).handle
                };
        ...
        static private IntPtr OnIconProc(IntPtr hWnd, int hObj, int uMsg, IntPtr wParam, IntPtr lParam, IntPtr pResult)
        {
            ...
            else if (uMsg == WM_PAINT)
            {
                ...
                if (ptr != IntPtr.Zero)
                {
                    ...
                    var icon = File.ReadAllBytes("Resources/icon.png");
                    var img = new ExImage(icon, icon.Length);
        ...
        static private IntPtr OnBkgProc(IntPtr hWnd, int hObj, int uMsg, IntPtr wParam, IntPtr lParam, IntPtr pResult)
        {
            ...
            else if(uMsg == WM_PAINT)
            {
                ...
                var douyinbkg = File.ReadAllBytes("Resources/douyinbkg.png");
                var img = new ExImage(douyinbkg, douyinbkg.Length);
                var canvas = new ExCanvas(ps.hCanvas);
